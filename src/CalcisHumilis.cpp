...
void CalcisHumilis::fillBlock(int16_t* dstLR, size_t nFrames) {
  ...
  const float sr = (float)cfg_.sampleRate;
  for (size_t i = 0; i < nFrames; ++i) {
    ...
    const float click = cfg_.clickAmt * c * sgn_soft(s);
    const float y = softClip((a * s + click) * cfg_.outGain);

    const int16_t L = sat16(y * gainL * 32767.0f);
    const int16_t R = sat16(y * gainR * 32767.0f);
